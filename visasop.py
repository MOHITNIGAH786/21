...
def gather_info():
    try:
        ...
        if page.status_code != 200:
            raise ValueError(f"Website not found: {website}")

        soup = BeautifulSoup(page.content, 'html.parser')
        program_description = soup.find("div", class_="program-description")

        if not program_description:
            raise ValueError(f"Program description not found on website: {website}")

        program_description = program_description.get_text()
